...
class EditForm(FlaskForm):
    task_index = HiddenField(validators=[DataRequired()])
    title = StringField(label='Название задачи', validators=[DataRequired(message='Название не может быть пустым')])
    done = HiddenField()
    ...
    if form.validate_on_submit():
        try:
            ...
            if 0 <= idx < len(tasks):
                ...
                flash('Неверный индекс задачи для удаления', 'danger')
        except ValueError:
            flash('Индекс задачи должен быть числом')
    else:
        flash('Не удалось подтвердить действие', 'danger')
    return redirect(url_for('index'))
...
def edit_task():
    ...
    if form.validate_on_submit():
        try:
            ...
            tasks = session.get('tasks', [])
            if 0 <= idx < len(tasks):
                tasks[idx]['title'] = form.title.data.strip()
                try:
                    done_value = int(form.done.data)
                    tasks[idx]['done'] = True if done_value == 1 else False
                except (ValueError, TypeError):
                    pass
                session.modified = True
                flash(f'Задача №{idx + 1} обновлена', 'success')
            else:
                flash('Неверный индекс задачи для редактирования', 'warning')
        except ValueError:
            flash('Индекс задачи должен быть числом', 'danger')
    else:
        flash('Не удалось подтвердить действие', 'danger')
